...
class AdminModulesControllerCore extends AdminController
{
    ...
    {
        $this->getPackageManager()->refreshPackages();
    }
    ...
    protected function extractArchive($file, $redirect = true)
    {
        try {
            ...
            if (count($directories) > 1) {
                $this->errors[] = sprintf(Tools::displayError('Package %1$s contains multiple top-level directories'), basename($file));
                return false;
            }

            if (count($directories) !== 1) {
                $this->errors[] = sprintf(Tools::displayError('Package %1$s does not contain valid module directory'), basename($file));
                return false;
            }
            ...
            if (!Validate::isModuleName($moduleName)) {
                $this->errors[] = sprintf(Tools::displayError('Package %1$s does not contain valid module'), basename($file));
                return false;
            }
